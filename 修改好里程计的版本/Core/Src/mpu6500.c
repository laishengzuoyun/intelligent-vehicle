...
uint8_t MPU6500_Init(I2C_HandleTypeDef *hi2c) {
    uint8_t check, data;
    // ���WHO_AM_I�Ĵ���
    if (HAL_I2C_Mem_Read(hi2c, MPU6500_ADDR, 0x75, 1, &check, 1, 100) != HAL_OK) {
        return 1; // I2C communication failed
    }
    if (check != 0x70 && check != 0x68) return 1; // 0x70/0x68ΪMPU6500/9250��ID

    // �������
    data = 0x00;
    if (HAL_I2C_Mem_Write(hi2c, MPU6500_ADDR, 0x6B, 1, &data, 1, 100) != HAL_OK) {
        return 1; // Failed to wake up MPU6500
    }

    // ���ü��ٶȺ����������̣���ѡ��Ĭ��2g/250dps��
    // data = 0x00; // ��2g
